...
export default function ExecutiveOverview({ 
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Top Welcome Banner */}
      <div className="glass-panel p-6 bg-gradient-to-r from-indigo-900/40 via-purple-900/20 to-slate-900/60 border-indigo-500/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-1">
            <Zap className="w-4 h-4 text-indigo-400 animate-pulse" />
            <span>Real-time Journey Stitching Platform</span>
          </div>
          <h2 className="text-2xl font-black text-white">Cross-Channel Customer Journey Intelligence</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-panel p-4 flex items-center space-x-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium">Stitched Events</p>
            <h3 className="text-2xl font-black text-white">{totalEvents}</h3>
            <p className="text-[11px] text-emerald-400 font-semibold flex items-center space-x-1 mt-0.5">
        ...
        <div className="glass-panel p-4 flex items-center space-x-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium">Active Profiles</p>
        ...
        <div className="glass-panel p-4 flex items-center space-x-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium">Avg Journey Friction</p>
            <div className="flex items-baseline space-x-2">
              <h3 className="text-2xl font-black text-amber-400">{avgFriction}</h3>
              <span className="text-xs font-semibold px-2 py-0.5 rounded bg-amber-500/20 text-amber-300">
                Elevated
              </span>
            </div>
            <p className="text-[11px] text-gray-400 mt-0.5">Configured 7-Factor Weight</p>
          </div>
        </div>
        ...
        <div className="glass-panel p-4 flex items-center space-x-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium">Active Escalations</p>
            <h3 className="text-2xl font-black text-rose-400">{openEscalations.length}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-panel p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-base font-bold text-white">Live Stitched Event Stream</h3>
            </div>
            <span className="text-xs text-gray-400 font-medium">Auto-stitching active</span>
          </div>
          ...
          <div className="space-y-2.5 max-h-[360px] overflow-y-auto pr-1">
            {events.map((evt) => (
              <div
                ...
                className="p-3 rounded-lg bg-gray-900/60 border border-white/5 hover:border-indigo-500/40 transition-all flex items-center justify-between cursor-pointer group"
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[10px] text-gray-400">({evt.time_display})</span>
                    </div>
                    <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">{evt.details}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3 shrink-0">
                  ...
                  <ChevronRight className="w-4 h-4 text-gray-500 group-hover:text-indigo-400 group-hover:translate-x-0.5 transition-all" />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="glass-panel p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-bold text-white">Channel Distribution</h3>
            <span className="text-xs text-gray-400">Events Volume</span>
          </div>
            ...
            {CHANNEL_METRICS.map(m => (
              <div key={m.channel} className="flex items-center justify-between text-xs">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-gray-300 font-medium">{m.channel}</span>
                </div>
                <div className="flex items-center space-x-3">
                  <span className="text-gray-400">{m.events} events</span>
                  <span className="text-amber-400 font-semibold">{m.friction_avg} friction</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="glass-panel p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-base font-bold text-white">High Friction & Churn Risk Customer Watchlist</h3>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {customers.slice(0, 3).map(customer => (
            <div
              ...
              className="p-4 rounded-xl bg-gray-900/80 border border-white/10 hover:border-indigo-500/50 transition-all cursor-pointer group"
            >
              <div className="flex items-center space-x-3">
                <img
                  src={customer.avatar}
              ...
              <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-xs">
                <div>
                  <span className="text-gray-400 block text-[10px]">Friction Score</span>
                  <span className={`font-black text-sm ${customer.friction_score > 70 ? "text-rose-400" : "text-amber-400"}`}>
                    {customer.friction_score} / 100
                  </span>
                </div>

                <div>
                  <span className="text-gray-400 block text-[10px]">Churn Risk</span>
