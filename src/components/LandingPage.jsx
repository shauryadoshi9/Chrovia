...
export default function LandingPage({ onLaunchApp, onOpenAuth, theme, onToggleTheme }) {
  const capabilities = [
    {
      icon: Network,
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)] text-[var(--text-main)] flex flex-col font-sans selection:bg-indigo-500 selection:text-white relative overflow-hidden transition-colors">
      
      {/* Dynamic Background Glow Effect */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-gradient-to-b from-indigo-600/10 via-purple-600/5 to-transparent blur-3xl pointer-events-none" />

      {/* Top Header Navigation */}
      <header className="sticky top-0 z-40 bg-[var(--bg-main)]/80 backdrop-blur-md border-b border-[var(--border-panel)] px-6 py-4 flex items-center justify-between transition-colors">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={onOpenAuth}
            className="px-4 py-2 rounded-lg bg-[var(--bg-inner)] hover:bg-[var(--bg-panel-hover)] text-[var(--text-main)] border border-[var(--border-panel)] text-xs font-bold transition-all cursor-pointer"
          >
            Sign In / Login
          </button>
          ...
          <button
            onClick={onLaunchApp}
            className="px-5 py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white text-xs font-extrabold flex items-center space-x-2 shadow-lg shadow-indigo-600/20 transition-all cursor-pointer"
          >
      ...
      <section className="px-6 py-16 md:py-24 max-w-6xl mx-auto text-center space-y-8 relative z-10">
        ...
        {/* Badge */}
        <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-600 dark:text-indigo-300 text-xs font-bold shadow-sm">
          <Zap className="w-4 h-4 text-indigo-500 animate-pulse" />
          <span>One Customer. Every Interaction. One Unified Journey.</span>
        </div>

        {/* Headline */}
        <h1 className="text-4xl sm:text-6xl font-black tracking-tight max-w-4xl mx-auto leading-tight text-[var(--text-main)]">
          Unify Fragmented Touchpoints Into{" "}
          <span className="text-indigo-600 dark:text-indigo-400">Actionable Journey Intelligence</span>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
          ...
          <button
            onClick={onOpenAuth}
            className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-[var(--bg-inner)] hover:bg-[var(--bg-panel-hover)] text-[var(--text-main)] border border-[var(--border-panel)] text-sm font-bold flex items-center justify-center space-x-2 transition-all cursor-pointer"
          >
        ...
        <div className="pt-8 max-w-5xl mx-auto">
          <div className="glass-panel p-3 border-indigo-500/30 shadow-2xl rounded-2xl relative overflow-hidden group">
            
            {/* Header bar mock */}
            <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--border-panel)] bg-[var(--bg-inner)] rounded-t-xl text-xs text-[var(--text-dim)] font-mono">
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full bg-rose-500" />
                <div className="w-3 h-3 rounded-full bg-amber-500" />
                <div className="w-3 h-3 rounded-full bg-emerald-500" />
                <span className="ml-2 text-indigo-600 dark:text-indigo-300 font-bold">chrovia-platform.internal</span>
              </div>
            ...
            <div className="p-6 bg-[var(--bg-main)] rounded-b-xl grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
              
              <div className="p-4 rounded-xl bg-[var(--bg-panel)] border border-[var(--border-panel)] space-y-2">
                <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold uppercase">Stitched Identity Graph</span>
                <h4 className="text-sm font-bold text-[var(--text-main)]">Aarav Shah (#1001)</h4>
                <p className="text-xs text-[var(--text-muted)]">Linked: Email, Phone, Loyalty ID, iOS Device Fingerprint</p>
                <div className="pt-2 flex items-center justify-between text-xs">
                  <span className="text-[var(--text-dim)]">Match Confidence</span>
                  <span className="font-extrabold text-emerald-600 dark:text-emerald-400">95% Verified</span>
                </div>
              </div>

              <div className="p-4 rounded-xl bg-[var(--bg-panel)] border border-[var(--border-panel)] space-y-2">
                <span className="text-[10px] text-amber-600 dark:text-amber-400 font-bold uppercase">Friction Score Solver</span>
                <h4 className="text-sm font-bold text-amber-600 dark:text-amber-400">Score: 84 / 100 (Critical)</h4>
                <p className="text-xs text-[var(--text-muted)]">3 Repeat Contacts, 1 SLA Breach, Mobile Payment Fail #504</p>
                <div className="pt-2 flex items-center justify-between text-xs">
                  <span className="text-[var(--text-dim)]">Churn Risk</span>
                  <span className="badge bg-rose-500/20 text-rose-600 dark:text-rose-300">87% CRITICAL</span>
                </div>
              </div>
      ...
      <section className="px-6 py-16 bg-[var(--bg-panel)] border-t border-b border-[var(--border-panel)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {capabilities.map((cap, i) => {
              ...
              return (
                <div
                  key={i}
                  className="p-6 rounded-2xl bg-[var(--bg-panel-hover)] border border-[var(--border-panel)] hover:border-indigo-500/40 transition-all space-y-3 glass-panel"
                >
                  <div className={`w-12 h-12 rounded-xl border flex items-center justify-center ${cap.bg} ${cap.color}`}>
                    <Icon className="w-6 h-6" />
                  </div>
                  <h3 className="text-base font-extrabold text-[var(--text-main)]">{cap.title}</h3>
      ...
      <section className="px-6 py-16 max-w-6xl mx-auto space-y-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-extrabold text-[var(--text-main)]">8 Core Dashboard Views</h2>
            <p className="text-xs text-[var(--text-muted)] mt-1">Product-like analyst workspace for real-time journey navigation</p>
          </div>
          ...
          {dashboardModules.map((mod, i) => {
            ...
            return (
              <div
                key={i}
                onClick={onLaunchApp}
                className="p-4 rounded-xl bg-[var(--bg-panel)] border border-[var(--border-panel)] hover:border-indigo-500/50 hover:bg-[var(--bg-panel-hover)] transition-all cursor-pointer group space-y-2"
              >
                <div className="flex items-center justify-between">
                  <Icon className="w-5 h-5 text-indigo-500 group-hover:scale-110 transition-transform" />
                  <span className="text-[10px] font-mono text-[var(--text-dim)]">Page {i + 1}</span>
                </div>
                <h4 className="text-sm font-bold text-[var(--text-main)] group-hover:text-indigo-600 dark:group-hover:text-indigo-300 transition-colors">{mod.name}</h4>
      ...
      {/* Footer */}
      <footer className="mt-auto border-t border-[var(--border-panel)] bg-[var(--bg-panel)] px-6 py-8">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-[var(--text-muted)]">
          <div className="flex items-center space-x-3">
            ...
            />
            <span className="font-extrabold text-[var(--text-main)]">CHROVIA Intelligence Platform</span>
            <span className="text-[var(--text-dim)]">|</span>
            <span>Academic Year 2026–27</span>
          </div>

          <div className="flex items-center space-x-4">
            <button onClick={onOpenAuth} className="hover:text-[var(--text-main)] transition-colors cursor-pointer">
              Sign In
            </button>
            <button onClick={onLaunchApp} className="text-indigo-600 dark:text-indigo-400 font-bold hover:underline transition-colors cursor-pointer">
              Launch Platform →
            </button>
          </div>
        </div>
